// mainsFreq for raspberry pico V2.0 270222 qrt@qland.de 
// display V2 version
//
// versions 
// 1.5      initial
...
#define MULTICORE   1                                   // 0 off, 1 on (0 off untested)
...
int main()
{
    ...
    EPaper::init(lut_ws_20_30);                         // first init
    
    EPaper::clearFrameMemory(0xff);                     // clear ePaper memory, 0=black, 1=white
    EPaper::displayFrame(POSTWAIT);                     // display and switch to second memory

    EPaper::clearFrameMemory(0xff);                     // both alternating buffers
    EPaper::displayFrame(POSTWAIT);                     // display and switch to first memory
...
void core1_entry()
{
    ...
    while(true){                                                // endless loop
        if(direrq){                                             // wait for display request
            while(EPaper::isBusy())                             // wait while display is busy
                tight_loop_contents();                          //

            if(once){                                           // only once
                EPaper::initFast(lut_full);                     // init fast

                EPaper::setFrameMemory(image);                  // send image to ePaper RAM (blocking)
                EPaper::displayFrame(POSTWAIT);                 // toggles frame memory

                EPaper::setFrameMemoryPartial(image);           // send image to ePaper RAM (blocking)
                EPaper::displayFramePartial(POSTWAIT);          // toggles frame memory
                ...
                EPaper::displayFramePartial(NOWAIT);            // display and toggle frame memory
            }
...
// -----------------------------------------------------------------------------
    ...
    void display()
    {
        ...
        if(EPaper::isBusy())                                // return if display is busy
            return;                                         // 
